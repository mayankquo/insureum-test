...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/insurer/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Insurer"
...
# call the script to create the channel, join the peers of insurer and org2,
...
function networkDown() {
  # stop org3 containers also in addition to insurer and org2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/insurer/msp organizations/fabric-ca/insurer/tls-cert.pem organizations/fabric-ca/insurer/ca-cert.pem organizations/fabric-ca/insurer/IssuerPublicKey organizations/fabric-ca/insurer/IssuerRevocationPublicKey organizations/fabric-ca/insurer/fabric-ca-server.db'
