...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"commonchannel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c commonchannel -s couchdb "
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from Insurer and Org2"
